...
namespace LINQ_Practice
{
    ...
    public class LINQ_Practice_MethodChaining
    {
        ...
        {
            var ActualCohorts = PracticeData.Where(cohort => cohort.PrimaryInstructor.LastName == "Zohan" || cohort.JuniorInstructors.Any(instructor => instructor.LastName == "Zohan")).ToList();
        ...
        {
            var ActualCohorts = PracticeData.Where(cohort => !cohort.FullTime && cohort.PrimaryInstructor.Active && cohort.JuniorInstructors.All(instructor => instructor.Active)).ToList();
        ...
        {
            var student = PracticeData.SelectMany(cohort => cohort.Students).OrderBy(students => students.Birthday).ToList()[0];
